...
namespace Proyecto_Nuevo_Avatar_V2.Pages.ADM9_Carreras
{
    public class CreateModel : PageModel
    {
        ...
        public CreateModel(ICarreraApiClient carreraApiClient, IInstitucionApiClient institucionApiClient, IProfesorApiClient profesorApiClient, ILoginApiClient loginApiClient)
        {
        ...
        [BindProperty]
        public Carrera NuevaCarrera { get; set; } = new();
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!Regex.IsMatch(NuevaCarrera.Nombre ?? "", @"^[A-Za-z¡…Õ”⁄·ÈÌÛ˙—Ò ]+$"))
            {
                ModelState.AddModelError("NuevaCarrera.Nombre", "El nombre solo puede contener letras y espacios.");
                return Page();
            }

            var (Exito, Mensaje, _) = await _carreraApiClient.CRUDCarreraAsync(NuevaCarrera, "Insert", token);
