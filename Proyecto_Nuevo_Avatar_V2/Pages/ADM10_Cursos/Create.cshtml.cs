...
namespace Proyecto_Nuevo_Avatar_V2.Pages.ADM10_Cursos
{
    public class CreateModel : PageModel
    {
        ...
        public CreateModel(ICursoApiClient cursoApiClient, ICarreraApiClient carreraApiClient, ILoginApiClient loginApiClient)
        {
        ...
        [BindProperty]
        public Curso NuevoCurso { get; set; } = new();
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!Regex.IsMatch(NuevoCurso.Nombre ?? "", @"^[A-Za-z¡…Õ”⁄·ÈÌÛ˙—Ò ]+$"))
            {
                ModelState.AddModelError("NuevoCurso.Nombre", "El nombre solo puede contener letras y espacios.");
                return Page();
            }

            var (Exito, Mensaje, _) = await _cursoApiClient.CRUDCursoAsync(NuevoCurso, "Insert", token);
